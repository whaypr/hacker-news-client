...
import cz.cvut.fit.bioop.hackernewsclient.commandExecution.commands.AppTimeToLiveCommand
...
class TimeToLiveFlagHandler(val executor: Executor, argIterator: Iterator[String]) extends FlagHandler {
  override def handle(flag: String): Option[Handler[String]] = {
    ...
    val arg = CommandLineParser.nextArg(argIterator).getOrElse(
      throw new Exception("Time-to-live must have a number value!")
    )
    val num = Utils.toInt(arg).getOrElse(
      throw new Exception("Time-to-live must be a number!")
    )

    executor.addCommand(new AppTimeToLiveCommand(num))
    None
  }
